        ...
        ${
          status === "online"
            ...
            : status === "offline"
              ...
              : "bg-yellow-100 border-yellow-300 text-yellow-700 dark:bg-yellow-700 dark:border-yellow-600 dark:text-yellow-300"
        } text-xs px-2 py-0.5
      `}
    >
      <span
        className={`mr-1.5 inline-block h-2 w-2 rounded-full ${
          status === "online" ? "bg-white" : status === "offline" ? "bg-gray-400" : "bg-yellow-400"
        }`}
      ></span>
  ...
  const mainContent = (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
        {statistics.map((stat) => (
          <Card key={stat.title} className="shadow-sm hover:shadow-lg transition-shadow duration-300">
            <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
              <CardTitle className="text-sm font-medium text-muted-foreground">{stat.title}</CardTitle>
              <stat.icon className={`h-5 w-5 text-${stat.color}-500 dark:text-${stat.color}-400`} />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-foreground">{stat.value}</div>
              <p className="text-xs text-muted-foreground pt-1">{stat.comparison} من الإجمالي</p>
            </CardContent>
            <CardFooter className="pt-1 pb-3">
              <MiniChart data={stat.trend} colorClassName={`bg-${stat.color}-500`} />
            </CardFooter>
          </Card>
        ))}
      </div>
      ...
      <Tabs defaultValue="notifications" className="w-full">
        ...
        <TabsContent value="notifications" className="space-y-6 mt-0">
          <div className="flex flex-col sm:flex-row items-center gap-2 mb-4">
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                { label: "البطاقات", type: "card", count: cardCount, icon: CreditCard },
                { label: "المتصلين", type: "online", count: onlineCountFiltered, icon: UserCheck },
              ].map((filter) => (
                <Button
                  key={filter.type}
                ...
                >
                  ...
                  {filter.label}
                  <Badge variant={filterType === filter.type ? "secondary" : "outline"} className="px-1.5 text-xs">
                    {filter.count}
                  </Badge>
                </Button>
              ))}
          ...
          <Card className="shadow-md overflow-hidden">
            <CardHeader className="px-4 py-3 border-b flex flex-row items-center justify-between bg-muted/30">
                        ...
                        <TableCell className="px-4 py-3">
                          <div className="flex flex-wrap gap-1.5">
                            ...
                            <Button
                              variant="link"
                              size="sm"
                              className={`p-0 h-auto text-xs ${notification.cardNumber ? "text-green-600 dark:text-green-400" : ""}`}
